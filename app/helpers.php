...
if (!function_exists('getWatingTime')) {
	...
	if(!function_exists('getTotalAppointment')){
        function getTotalAppointment() {
            return Appointments::select('id')->whereIn('app_click_status',array(5,6))->where("delete_status",1)->where("added_by","!=",24)->count();
		}
	}
  ...
  if (!function_exists('getLabReportById')) {
    function getLabReportById($order_id) {
		...
		if(isset($data) && $data->company_id != 0) {
			$data['report_pdf_name'] = url("/")."/public/others/lab-reports/".$data->report_pdf_name;
		}
		return $data;
    }
}
   ...
   if (!function_exists('getThyrocarePackageGroup')) {
    function getThyrocarePackageGroup($type=null) {
      ...
      $items = array();
          ...
          // foreach($product as $structs){
                if ($group->group_name == $structs->disease_group) {
  ...
  if (!function_exists('getLabCart')) {
    function getLabCart($user_id = null) {
      ...
      foreach ($labcart as $key => $cart) {
        ...
        elseif ($cart->product_type == 'PROFILE' || $cart->product_type == 'POP') {
          $product = File::get(public_path('thyrocare-data/Profile.txt'));
          $products = json_decode($product, true);
        }
        elseif ($cart->product_type == 'TEST') {
          $product = File::get(public_path('thyrocare-data/Tests.txt'));
          $products = json_decode($product, true);
        }*/
			if(count($products) > 0) {
...
if (!function_exists('getLabCartNew')) {
  ...
  if (!function_exists('getLabOrdersCount')) {
     function getLabOrdersCount($type) {
       ...
       //all Orders
       if ($type == 1) {
			$campData = CampData::where(["user_id" => $user_id])->count();
			$ordersCount = LabOrders::where(["user_id" => $user_id])->count();
			$ordersCount = $campData + $ordersCount;
       }
        //Upcomming Orders
       elseif ($type == 2) {
	...
	if (!function_exists('getTitleBySlug')) {
		function getTitleBySlug($info_type = null,$slug = null) {
			...
			if($info_type == "Speciality"){
				...
				$speciaity = Speciality::select(["keywords","description"])->where('slug',$slug)->first();
				if(!empty($speciaity)) {
					...
					// if($keywords) {
						...
						// }
					// }
					$keyword = $speciaity->keywords." in ".$cName." | Health Gennie";
...
if (!function_exists('checkPatientCount')) {
	function checkPatientCount($id) {
		$result = UsersSubscriptions::with("UserSubscribedPlans")->where('user_id',$id)->where('order_status','1')->first();
		 //pr($result);
		$max_patient_count = 0;
		if(!empty($result)){
...
if(!function_exists('getDoctorDocumentType')) {
 function getDoctorDocumentType($type=null) {
	$data = array('1' => 'Profile Picture','2' => 'Clinic Picture','3' => 'Registration Certificate','4' => 'Degree','5' => 'Address Proof');
	if(!empty($type)) {
		$data = $data[$type];
	}
	return $data;
...
if(!function_exists('getDocTypeClass')) {
 function getDocTypeClass($type) {
  $data = array('1' => 'profilePic','2' => 'clinicPic','3' => 'regCet','4' => 'degree','5' => 'addPrf');
  if (!empty($type)) {
	$data = $data[$type];
  }
  return $data;
...
if (!function_exists('writeClinicNoteFile')) {
  function writeClinicNoteFile($patient_number,$prescription) {
...
if (!function_exists('getPresFile')) {
  function getPresFile($appId) {
	...
	if(!empty($presDta->prescription) && $presDta->type == "1") {
		writeClinicNoteFile($presDta->patient_number,$presDta->prescription);
		$preUrl = 	url("/")."/public/PatientDocuments/".$presDta->patient_number."/misc/clinicalNotePrint.pdf";
		return $preUrl;
	}
  }
}
...
if(!function_exists('getAllThyrocareData')) {
    function getAllThyrocareData() {
		...
		// $ofr_arr = File::get(public_path('thyrocare-data/Offer.txt'));
		// $arr2 = json_decode($ofr_arr, true);
		// $mArr = array_unique(@array_merge($arr1,$arr2),SORT_REGULAR);
			
        // $arr3 = json_decode(File::get(public_path('thyrocare-data/Profile.txt')),true);
		// $mArr2 = array_unique(@array_merge($mArr,$arr3),SORT_REGULAR);
		
		
		// $arr4 = json_decode(File::get(public_path('thyrocare-data/Tests.txt')),true);
		// $products = array_unique(@array_merge($arr3,$arr4),SORT_REGULAR);
...
function getUserIdByRefCode($refCode){
...
if(!function_exists('getSubscriptionData')){
	...
	{
		$data = UsersSubscriptions::all()->count();
		return $data;
	}
