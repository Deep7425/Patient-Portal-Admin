...
public function createLaborderforcsv($data,$userData) {
   ...
   if($data['date']){
    ...
    $data['date']= Carbon::instance(\PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject( $data['date']));
    ...
    $user_array['organization'] = (isset($data['organization'])) ? $data['organization'] : null;
    ...
    LabOrders::where(["orderId"=>$orderId])->update([
        ...
        'status' => 1,
        'org_id' =>  $user_array['organization']
    ]);
    $product_name = [];
