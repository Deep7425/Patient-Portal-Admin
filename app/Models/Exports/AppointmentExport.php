...
class AppointmentExport implements FromCollection,WithHeadings,ShouldQueue {
	use Exportable;
	private $data;
	...
	public function headings(): array {
        return ['Sr. No.','Appointment Date','Time','Order ID','Disease','Lab Test','Doctor Name  (Id) (Mobile)','Patient Name  (Pid) (Mobile)','Gender/Age','Type','Doc Fee To Pay','Consultation Fee (Rs.)','Total Pay (Rs.)','Payment Status','From','Rating','Organization','Created At','Status','Payment Status','Appointment Status','Waiting Time Hour:Minut'];
    }
	...
	public function setValues($appointments){
		...
		foreach($appointments as $i => $element) {
			...
			$userLabs = "";
			if(count($element->PatientLabs) > 0) {
			...
			$appointmenArray[] = array(
				 ...
				 date('H:i:s',strtotime($element->start)),
				 @$appointmentOrder->id,
				 getChiefComplaints($element->chiefComplaints),
				 $userLabs,
				 @$user->DoctorInfo->first_name." ".@$user->DoctorInfo->last_name." (".@$user->id.") (".@$user->DoctorInfo->mobile.")".$accNo,
				 ...
				 $docFeeToPay,
				 (!empty($appointmentOrder))? @$appointmentOrder->order_subtotal : $element->consultation_fees,
				 (!empty($appointmentOrder))? @$appointmentOrder->order_total : $element->consultation_fees,
				 @$paymentstatus,
				 @$bookfrom,
				 (!empty($appointmentOrder->rating)) ? @$appointmentOrder->rating." STAR" : "",
				 (!empty($element->UserPP)) ? @$element->UserPP->OrganizationMaster->title : "",
