	...
	public function findDoctorCityByType(Request $request,$city,$type = null,$name = null) {
		...
		if(!empty($speciaity)) {
			...
			if(count($infoData) <= 0 && !empty($speciaity) && !empty($city)){
				$infoData = Doctors::with(["docSpeciality","DoctorRatingReviews"])->where(["delete_status"=>1,'status'=>1,'varify_status'=>1])->whereNotNull('speciality')->where("oncall_status","!=",0)->where('city_id',$city->id)->whereIn('speciality',$s_ids)->get();
			}
			$infoData = dataSequenceChange($infoData,$speciaity->id);
			// $docs_arr = [];
			// foreach($infoData as $info){
				// if($info->speciality == $speciaity->id){
					// $docs_arr[] = $info;
				// }
			// }
			// foreach($infoData as $info){
