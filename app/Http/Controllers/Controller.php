...
class Controller extends BaseController
{
	...
	public function sendSMS($mobile,$message,$tempId = null) {
		try{
		if($tempId != null) {
			  $url = "http://smsdigital.in/httpapi/smsapi?uname=healthgennie&password=hgennie123&sender=GENNIE&receiver=".$mobile."&route=T2&msgtype=1&sms=".$message."&tid=".$tempId;
		...
		// $url = "http://smsdigital.in/httpapi/smsapi?uname=healthgennie&password=hgennie123&sender=GENNIE&receiver=".$mobile."&route=T2&msgtype=1&sms=".$message."";
		  $url = "http://sms.ringinginfotech.in/V2/http-api.php?apikey=apw4VK3xVRvoHISh&senderid=SABHWL&number=".$mobile."&message=".$message;
		  $json = json_decode(file_get_contents($url),true);
		  if($json) {
			return 1;
		  }
    ...
    public function getUpdateCityList($id)
    {
       ...
       return $cities;
		...
		public	function getCurrentLocality(Request $request) {
			...
			try{
			$city_name = ''; $state_name = ''; $city_id = ''; $state_id = '';$locality_area = '';
			...
			if(!empty($request->city_name) || !empty($request->state_name)) {
				...
				if(!empty($city_name)){
					...
					$city_id = $query_city->whereHas('State', function($qr) {$qr->Where(['country_id'=>'101']);})->first();
				}
				if(!empty($city_id)) {
			...
			if(!empty($request->locality_area)) {
				...
				//$state_data = State::with("Country")->where('country_id','101')->where('name','like', '%'.$locality_area.'%')->get();
				$city_data = City::with(["State"])->where('name','like', '%'.$locality_area.'%')->whereHas('State', function($qrr) {$qrr->Where(['country_id'=>'101']);})->get();
			}
			...
			if(!empty($state_data) && count($state_data) > 0){
	...
	public function androidNotificationSend($apikey = null,$registrationIds=[],$message=null,$title=null,$subtitle=null,$tickerText=null,$page=null) {
		...
		try{
			...
			// $txt = "testst";
			// $myfile = file_put_contents('public/testData/test.txt', $txt.PHP_EOL , FILE_APPEND | LOCK_EX);
			// File::put(public_path('testData/test.txt'),$arr);
			die;
	...
	public function paymentResponse(Request $request) {
		...
		if($response['merchant_param1'] == "HealthGennie Lab Order") {
			...
			if($response['order_status'] == 'Success') {
				...
				/*$lab = LabOrders::select(["user_id"])->where(["orderId"=>$response['order_id']])->first();
				...
				$order_array = array(
					'api_key' => $response['merchant_param2'],
					'VisitId' => $response['order_id'],
					'Pincode' => $response['merchant_param3'],
					'AppointmentDate' => $response['merchant_param4'],
				);
				...
				curl_close($ch_app);
				$app_output = json_decode($app_output,true);*/
			...
			if($response['STATUS'] == 'TXN_SUCCESS') {
				...
				if(!empty($output) && $output['respId'] == 'RES02012') {
					...
					$forder_array = array( 
						...
						'VisitId' => $response['order_id'],
						'Pincode' => $meta_data->pincode,
						'AppointmentDate' => $meta_data->appt_date,
					);
	...
	public function paytmResponseMIniApp(Request $request) {
		...
		if($response['STATUS'] == 'TXN_SUCCESS') {
			...
			//return \Redirect::to('https://www.healthgennie.com/appointment/cancel?order_id='.base64_encode($TXNID));
		...
		if($this->is_connected()==1) {
			if($type == 3){
				...
				/*if(!empty($appointment->Patient->email)) {         
					...
					if($EmailTemplate && !empty($to) && checkAppointmentIsElite($appointment->id) == 0) {
				...
				if(!empty($appointment->Patient->mobile_no)) {
					// if(checkAppointmentIsElite($appointment->id) == 1) {
						if($orderType == 0) {
	...
	public function notificationDoctorForAppointment($appointment=null) {
		...
		/*public	function updateLocalityByName(){
			// $localities = DB::table('city_localities')->where(["city_id"=>3378])->get();
			
			// if(count($localities) > 0) {
				// foreach($localities as $locs) {
					// if(!empty($locs->name)) {
						// $doctor = DB::table('doctors')->select(["id","locality_id"])->where("address_1",'Kalwar Road')->first();
						
						// if(!empty($doctor) && empty($doctor->locality_id)) {
							// $arr[] = $doctor->id;
							// DB::table('doctors')->where("id",$doctor->id)->update(["locality_id"=>$locs->id,"locality_name"=>$locs->name]);
						// }
					// }
				// }
			// }
			$doctors = DB::table('doctors_live_with_crm_data')->select(["id","locality_id","address_1","city_id"])->get();
			$arr = [];
			foreach($doctors as $doctor) {
				if(!empty($doctor) && !empty($doctor->address_1) && empty($doctor->locality_id)) {
					$qry = DB::table('city_localities')->select(["id","name"]);
					if(!empty($doctor->city_id)) {
						$qry->where(["city_id"=>$doctor->city_id]);
					}
					$localities = $qry->get();
					if(count($localities) > 0){
						foreach($localities as $local){
							if (strpos($doctor->address_1, $local->name) == true) {
								// $arr[] = $doctor->id;
								DB::table('doctors_live_with_crm_data')->where("id",$doctor->id)->update(["locality_id"=>$local->id,"locality_name"=>$local->name]);
								break;
							}
						}
					}
					// $loc = DB::table('city_localities')->select(["id","name"])->where("name",'LIKE',$doc->address_1)->first();
					// $arr[] = $loc->id;
				}	
			}
			// dd($arr);
			...
			if(!empty($practiceId)) {
				...
				if($this->is_connected()==1) {
					if($EmailTemplate) {
					   $body = $EmailTemplate->description;
					   $password = $pass;
					   $mailMessage = str_replace(array('{{username}}','{{clinic_name}}', '{{password}}','{{to}}'),
					   array($username,$practiceData->clinic_name,$password,$to),$body);
					   $datas = array( 'to' =>$to,'from' => 'noreply@healthgennie.com','mailTitle'=>$EmailTemplate->title,'practiceData'=>$practiceData,'content'=>$mailMessage,'subject'=>$EmailTemplate->subject);
						   try
						   {
							 Mail::send( 'emails.mailtempPractice', $datas, function( $message ) use ($datas) {
				  ...
				  if($EmailTemplate && !empty($to)) {
					  ...
					  if(!empty($PracticesSubscriptions)){
						  foreach($PracticesSubscriptions->SubscribedPlans as $SubscribedPlan) {
							  $type = "";
							  if($SubscribedPlan->plan_duration_type == "d"){ $type = "Day"; }
							  elseif($SubscribedPlan->plan_duration_type == "m") { $type = "Month"; }  elseif($SubscribedPlan->plan_duration_type == "y"){ $type =  "Year"; }
					 ...
					 $tbl .= "</tbody></table>";
					  $mailMessage = str_replace(array('{{name}}','{{date}}','{{subscribetable}}'),
					  array($username,$today_date,$tbl),$body);
	...
	public function sendMedicineOrderMail($order_id,$date) {
		try{
		  ...
		  if(!empty($order)) {
			  ...
			  if($this->is_connected()==1) {
					if(!empty($user->mobile_no)) {
					  // $messagecamp = urlencode("Dear ".ucfirst($username).", Congratulations. Your health gennie medicine order ".$order_id." has been confirmed by our partner and will be delivered by ".$date."\nThanks Team Health Gennie");
					  // $this->sendSMS($user->mobile_no,$messagecamp);
					}
				}
			}
			  ...
			  if(!empty($PracticesSubscriptions)) {
				...
				$username = $user->practiceDetails->first_name." ".$user->practiceDetails->last_name;
				 ...
				 if($EmailTemplate) {
				   ...
				   $mailMessage = str_replace(array('{{id}}','{{name}}','{{email}}','{{mobile}}','{{msg}}','{{subscribetable}}'),
				   array($user->id,$username,$user->email,$mobile,$msg,$tbl),$body);
	  ...
	  public function writeThyrocareData(Request $request) {
		  ...
		  try{
			...
			if($LoginData){
				...
				// pr("https://www.thyrocare.com/APIS/master.svc/".$api_key."/OFFER/products");
				// $thyrocate_data_all = file_get_contents("https://www.thyrocare.com/API_BETA/master.svc/".$api_key."/ALL/products");
				//$thyrocate_data_all = file_get_contents("https://www.thyrocare.com/APIS/master.svc/".$api_key."/ALL/products");
				...
				// $thyrocate_data_offer = file_get_contents("https://www.thyrocare.com/API_BETA/master.svc/".$api_key."/OFFER/products");
				//$thyrocate_data_offer = file_get_contents("https://www.thyrocare.com/APIS/master.svc/".$api_key."/OFFER/products");
				//$thyrocate_data_offer = file_get_contents("https://www.thyrocare.com/APIS/master.svc/".$api_key."/ownoffer/products");
				...
				//$thyrocate_data_profile = file_get_contents("https://www.thyrocare.com/APIS/master.svc/".$api_key."/PROFILE/products");
				 // $thyrocate_data_profile = file_get_contents("https://www.thyrocare.com/API_BETA/master.svc/".$api_key."/PROFILE/products");
				...
				// $thyrocate_data_tests = file_get_contents("https://www.thyrocare.com/API_BETA/master.svc/".$api_key."/TESTS/products");
				//$thyrocate_data_tests = file_get_contents("https://www.thyrocare.com/APIS/master.svc/".$api_key."/TESTS/products");
	 ...
	 public function getthyrocareLab($type,$limit = null) {
		...
		try{
			...
			if(!empty($limit)) {
				$product = array_slice($product, 0, $limit);
			}*/
			return $products;
	  ...
	  public function getFavLabList() {
		  ...
		  try{
			$thyProducts = [];
			// $thyProductsProfile = $this->getthyrocareLab("PROFILE",4);
			// if(count($thyProductsProfile) > 0){
				// foreach($thyProductsProfile as $product){
					// $thyProducts[] = $product;
				// }
			// }
	...
	public function fetchLabDataNew($user_array) {
	 ...
	 try{	
		...
		$c_labs = LabCollection::with("DefaultLabs","LabCompany")->where('delete_status', '=', '1')->whereHas("DefaultLabs",function($q) use($search) {
	...
	public function updateSlugForDoctor(Request $request) {
		//dd($_SERVER);
		//saveUserActivity($request, 'test', 'doctors', 2);
		//$doctors = Doctors::select(["id","user_id","practice_id"])->whereNotNull("user_id")->get();
		//foreach($doctors as $doc){
			// $feedback = PatientFeedback::where('doc_id',$doc->id)->first();
			// if(empty($feedback)) {
			...
			// }
			// else{
				// if(checSubcriptionStatus($doc->practice_id) == 1) {
					// PatientFeedback::where(["doc_id"=>$doc->id])->update([
						// 'doc_type'=>2
					// ]);
				// }
			// }
		//}
		// $slugs = DoctorSlug::whereNotNull('practice_id')->get();
	
		// foreach($slugs as $slg) {
			// $slugg = DoctorSlug::where(["practice_id"=>$slg->practice_id])->first();
			// DoctorSlug::where(["id"=>$slg->id])->update(["clinic_name_slug"=>$slugg->clinic_name_slug]);
		// }
		
		// $cities = CityLocalities::groupBy('slug')->havingRaw('count(*) > 1')->get();
		// dd($cities);
		// foreach($cities as $city) {
			// $slug = $city->slug."-".$this->clean(getCityName($city->city_id));
			// CityLocalities::where(["id"=>$city->id])->update(["slug"=>$slug]);
		// }
		//$specialities = Speciality::get();
		//$arr = [];
		/*foreach($specialities as $spe){
			$slug = trim(strtolower($spe->specialities));
			...
			$slug = str_replace(")","", $slug);
			//$arr[] = $slug;
			Speciality::where(["id"=>$spe->id])->update(["slug"=>$slug]);
		}*/
		/*$slugs = DoctorSlug::groupBy('clinic_name_slug')->havingRaw('count(*) > 1')->get();
		
		foreach($slugs as $slg) {
			$slug_exists = DoctorSlug::where(["clinic_name_slug"=>$slg->clinic_name_slug])->get();
			if(count($slug_exists) > 0) {
				foreach($slug_exists as $i => $dd) {
					$i+1;
					$slugg = $dd->clinic_name_slug."-".$i;
					// $name = $this->clean(getDoctorNameById($dd->doc_id));
					// $clinic_slug = str_replace(' ',"-",$name."-".str_replace('-',' ',$dd->clinic_name_slug));
					DoctorSlug::where(["id"=>$dd->id])->update(['clinic_name_slug'=>trim(strtolower($slugg))]);
				}
			}
			// break;
		}*/
		// $doctors = Doctors::with("docSpeciality")->select(["id","first_name","last_name","city_id","speciality","clinic_name","practice_type","locality_id"])->get();
		...
		/*if(count($doctors) >0) {
			foreach($doctors as $data) {
				...
				if(!empty($data->docSpeciality)) {
					$slug = "dr-".$name."-".$data->docSpeciality->slug;
				}
				else{
					if(!empty($data->getCityName)){
						$slug = "dr-".$name."-".str_replace(' ', '-',strtolower(trim($data->getCityName->name)));
					}
					else{
						$slug = "dr-".$name;
					}
				...
				$doctor_exists = DoctorSlug::where(["name_slug"=>$slug,"city_id"=>$data->city_id])->first();
				if(!empty($doctor_exists)) {
					...
					DoctorSlug::create(['doc_id'=>$data->id,'name_slug' => strtolower($slug),"city_id"=>$data->city_id]);
				}
				else{
					DoctorSlug::create(['doc_id'=>$data->id,'name_slug' => strtolower($slug),"city_id"=>$data->city_id]);
				}
		...
		if(count($doctors) >0) {
			foreach($doctors as $data) {
				...
				if(!empty($data->getCityName) && !empty($data->CityLocalities)) {
					$slug = $name."-".str_replace(' ', '-',strtolower(trim($data->getCityName->name)))."-".str_replace(' ', '-',strtolower(trim($data->CityLocalities->name)));
				}
				else if(!empty($data->getCityName)) {
					$slug = $name."-".str_replace(' ', '-',strtolower(trim($data->getCityName->name)));
				}
				else{
					if(!empty($data->docSpeciality)) {
						$slug = $name."-".str_replace(' ', '-',strtolower(trim($data->docSpeciality->slug)));
					}
					else{
						$slug = $name;
					}
				...
				$doctor_exists = DoctorSlug::where(["clinic_name_slug"=>$slug,"city_id"=>$data->city_id])->first();
				if(!empty($doctor_exists)) {
					$name = $data->clinic_name;
					$name = $this->clean($name);
					...
					if(!empty($data->getCityName) && !empty($data->CityLocalities)) {
						...
						if(!empty($data->docSpeciality)) {
							$slug = $name."-".$data->docSpeciality->slug;
						}
						else{
							$slug = $name;
						}
					}
					DoctorSlug::where(["doc_id"=>$data->id])->update(['doc_id'=>$data->id,'clinic_name_slug' => strtolower($slug),"city_id"=>$data->city_id]);
				}
				else{
					DoctorSlug::where(["doc_id"=>$data->id])->update(['doc_id'=>$data->id,'clinic_name_slug' => strtolower($slug),"city_id"=>$data->city_id]);
				}
	...
	// public function checkAppointmentOrderStatus(Request $request) { 
		/*$working_key = '5D52E44A54B1FA6DD1CCB5B1AEA2B71F'; //Shared by CCAVENUES
		$access_code = 'AVLA78FF46AM51ALMA';
        $orders = AppointmentOrder::where(["order_status"=>"0","type"=>"1"])->whereDate('created_at', Carbon::today())->get();
		if(count($orders)>0) {
			//pr($orders);
			foreach($orders as $order) {
				//echo $order->id.'kaps';
				$merchant_json_data = array('order_no' => $order->id);
				...
				//echo "<pre>";
				// print_r($obj['Order_Status_Result']);
				if($obj['Order_Status_Result']['status'] == 0){ 
				 
				
					if($obj['Order_Status_Result']['order_status'] == 'Shipped' || $obj['Order_Status_Result']['order_status'] == 'Successful'){
						//echo $order->id.$obj['Order_Status_Result']['order_status'].'kkkl';
						if(!empty($order->meta_data)) {  
							//$this->putAppointmentDataApp($order,'',$obj['Order_Status_Result']);
						}
					}
                }				
				//die;
	...
	public function loginUserByPaytmData(Request $request) {
		if ($request->isMethod('post')) {
			$ress = null; $response = null; $user = null; $success = 0; $content = "";
			$code = $request->code;
			$paytmParams = array();
	...
	public function enquiryFromSubmit(Request $request) {
	  try{	
		...
		$user = EnquiryForm::create([
			...
			if(!$request->ajax()){
				return redirect()->route('subscriptionAds');
            }
			return ['status'=>1];
		}
		return ['status'=>2];
