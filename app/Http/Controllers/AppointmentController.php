...
class AppointmentController extends Controller
{
	...
	public function userAppointment(Request $request) {
		...
		if(!empty($user->pId)){
			...
			$appointments = Appointments::with(['patient','visitType','practiceDetails','user.doctorInfo.docSpeciality'])->whereIn('pID',$p_ids)->where("delete_status",1)->orderBy('start','DESC')->paginate(10);	
	...
	public function appointmentDetails(Request $request) {
		...
		if(isset($files) && count($files)){
		 foreach($files as $file){
			...
			$documents[] = ['doc_name'=>$docName,'document'=>getPath($file)];
		 }
		}
		 // pr($documents);
		// if(is_dir($path."/".$appointment_id)) { 
			// $file_name = array_slice(scandir($path."/".$appointment_id) , 2);
			// foreach ($file_name as $file_nm) {
				// if(!empty($file_nm) && $file_nm != "htmltopdfview.pdf"){
					// $documents[] = array("document" => $path_url."/appointments/".$appointment_id."/".$file_nm,'doc_name'=>$file_nm);
				// }
			// }
		// }
