...
class HomeController extends Controller {
					...
					if($user = \DB::table("admins")->where($userdata)->first()){
						if($user->status == '1'){
						   if (Hash::check(Input::get('password'), $user->password)){
								...
								return redirect("admin/")->with('error_msg', "Invalid credentials")->send();
							}
						}
						else{
                         ...
                         return redirect("admin/")->with('error_msg', "Invalid username")->send();
                    }
                }
            }
	...
	// 	$previousFiveYears = $currentYear - 5;

	// 	$data = UsersSubscriptions::where(DB::raw('YEAR(created_at)'), '>=', $previousFiveYears)->get();
	...
	// 	return view('admin.index',compact('admin' , 'data'));
	// }
	...
	// dd($currentYear);
		...
		// If the start month is less than 1, adjust the year accordingly
		if ($startMonth < 1) {
			$startMonth += 12;
			$startYear--;
		}
		
		// Initialize an array to store data counts for the last three months
		$dataCounts = [];
		...
		for ($i = 0; $i < 3; $i++) {
			...
			// Retrieve data count for the current month
			$count = UsersSubscriptions::whereYear('created_at', $year)
				->whereMonth('created_at', $month)
				->count();
			
			// Add count to the dataCounts array
			$dataCounts[] = $count;
		}
	
		// Generate labels for the last three months
		$labels = [];
		for ($i = 0; $i < 3; $i++) {
			// Adjust month and year if necessary
			$month = $startMonth + $i;
		...
		// Reverse the arrays to maintain correct order
		$dataCounts = array_reverse($dataCounts);
		...
		if ($request->input('is_live')!= "") {
         ...
         return redirect()->route('admin.doctorsList',$params)->withInput();
			...
			if(!empty($search)) {
				...
				$query->where(DB::raw('concat(first_name," ",last_name," ",mobile_no," ",IFNULL(clinic_name,""))') , 'like', '%'.$search.'%');
			}
	...
	public  function nonClaimDoctorsList(Request $request) {
		...
		$params = array();
         ...
         return redirect()->route('admin.nonClaimDoctorsList',$params)->withInput();
			...
			if($spaciality){
				$query->whereHas('docSpeciality', function($q)  use ($spaciality) {$q->Where('specialities','like','%'.$spaciality.'%');});
			}
			$page = 25;
			if(!empty($request->input('page_no'))){
				$page = base64_decode($request->input('page_no'));
	...
	public  function patientList(Request $request) {
		...
		$params = array();
		 ...
		 if ($request->input('organization_type') != "") {
			 $params['organization_type'] = base64_encode($request->input('organization_type'));
		 }
         return redirect()->route('admin.patientList',$params)->withInput();
		}
		else {
			$search = base64_decode($request->input('search'));
			$query = User::with(['OrganizationMaster','getCityName','State','UsersSubscriptions'])->select('id','pId','device_type','login_type','first_name','last_name','gender','mobile_no','other_mobile_no','address','city_id','state_id','organization','location_meta','created_at','note')->where(['parent_id'=>0]);
			if(!empty($search)){
			...
			if(!empty($request->input('organization_type'))){
            ...
            if(base64_decode($request->input('file_type')) == "excel") {
				 $patData = $query->orderBy("id","DESC")->get();
					...
					foreach($patData as $i => $element) {
						if($user_type == '5') {
							if(!empty($element->pId) && $element->tot_appointment > 0){
								$patDataArr[] = $element;
							}
						}
						else{
				...
				foreach($patData as $i => $pat) {
						...
						if(!empty($pat->location_meta)){
							...
							if(!empty($location_meta->locality)){ $locality = $location_meta->locality;
							}
							if(!empty($location_meta->subAdministrativeArea)){ $subAdministrativeArea = $location_meta->subAdministrativeArea;
							}
							if(!empty($location_meta->administrativeArea)){ $administrativeArea = @$location_meta->administrativeArea;}
						}
						$patDataArray[] = array(
							$i+1,
							(!empty($pat->pId)) ? $pat->tot_appointment : 0,
		...
		return view('admin.Patients.patient-list',compact('patients','AllPatientsIds','OrganizationList','practices','plans','labpackages','symptomslist'));
		}
		catch (\Exception $e) {
			dd($e);
		}
	 ...
	 public function feedbackPatAll(Request $request) {
		...
		$params = array();
         ...
         return redirect()->route('admin.feedbackPatAll',$params)->withInput();
			...
			$filters = array();
			$search = base64_decode($request->input('search'));
			$query = PatientFeedback::with(['Doctors'])->whereNotNull("user_id");
			...
			if($request->input('file_type') == "excel") {
				...
				foreach($feedbacks as $i => $enq) {
					...
					if($enq->recommendation == 1){ $ab='Less than 5 min'; }
					if($enq->recommendation == 2) { $ab='5 min to 10 min'; }
					if($enq->recommendation == 3) { $ab='10 min to 30 min'; }
				    if($enq->recommendation == 4) { $ab='30 min to 1 hour'; }
					if($enq->recommendation == 5){ $ab='More than 1 hour'; }
					   if($enq->recommendation == 1){ $d='Yes'; }else{ $d='No'; }
					   if(!empty($enq->user_id)){ $DocName=getUserName($enq->user_id); }
					   if($enq->visit_type == '1'){ $varFollow='Consultation';} 
					   if($enq->visit_type == '2') { $varFollow='Procedure'; } 
					   if($enq->visit_type == '3') { $varFollow='Follow up'; }
   ...
   public function subcribedAll(Request $request) {
	 ...
	 public function contactQuery(Request $request) {
		...
		$params = array();
         ...
         return redirect()->route('admin.contactQuery',$params)->withInput();
			...
			$filters = array();
			$search = base64_decode($request->input('search'));
			$query = Contact::where(["delete_status"=>1])->orderBy('id', 'desc');
	...
	public function enquiryQuery(Request $request) {
		...
		$params = array();
         ...
         return redirect()->route('admin.enquiryQuery',$params)->withInput();
			...
			$filters = array();
			$search = base64_decode($request->input('search'));
			$query = Enquiry::with('User.UsersSubscriptions')->orderBy('id', 'desc');
            if(!empty($search)) {
				$query->where('name', 'like','%'.$search.'%');
			}
			if(!empty($request->input('start_date')) || !empty($request->input('end_date'))) {
	...
	public function corporateLeads(Request $request) {
		...
		$params = array();
         ...
         return redirect()->route('admin.corporateLeads',$params)->withInput();
			...
			$filters = array();
			$search = base64_decode($request->input('search'));
			$query = Corporate::orderBy('id', 'desc');
            if(!empty($search)) {
				$query->where('name', 'like','%'.$search.'%');
			}
			if(!empty($request->input('start_date')) || !empty($request->input('end_date'))) {
    ...
    public  function sponsoredDoctor(Request $request) {
  		...
  		else
       {
        ...
        if(!empty($search)){
          $query->whereHas('Doctors', function($q)  use ($search) {$q->Where('clinic_name','like','%'.$search.'%');});
        }
  			if(!empty($user_id)){
  				$query->Where('user_id',$user_id);
  			}
...
public function sendUserBulkSms(Request $request) {
	...
	public function userOtpList(Request $request) {
		...
		$params = array();
         ...
         return redirect()->route('admin.userOtpList',$params)->withInput();
			...
			$filters = array();
			$search = base64_decode($request->input('search'));
			$query = UsersOTP::select(["otp","mobile_no","created_at","updated_at"])->whereNotNull("otp");
			if(!empty($search)) {
				$query->where('mobile_no','like','%'.$search.'%');
			}
			if(!empty($request->input('start_date')) || !empty($request->input('end_date'))) {
			...
			if($file_type == "excel") {
				 ...
				 foreach($doctors as $i => $doc) {
					...
					// elseif(substr($doc->member_id,0,3) =='Doc') {
						// $typee = "Doctor";
					// }
					$types = []; if(!empty($doc->oncall_status)){ $types = explode(',',$doc->oncall_status); }
					if(in_array(1,$types) && in_array(2,$types)) { $typee = "BOTH";  }
					elseif(in_array(2,$types)) { $typee = "In-clinic"; } elseif(in_array(1,$types)){ $typee = "Tele"; }
					$doctorArray[] = array(
						 $i+1,
						 $doc->id,
						 $typee,
         ...
         return redirect()->route('admin.corporateUsers',$params)->withInput();
            ...
            if(base64_decode($request->input('file_type')) == "excel") {
				 $patData = $query->orderBy("id","DESC")->get();
					...
					$patData = $patDataArr;
					$patDataArray[] = array('Sr. No.','Appointment','Registered Type','Name','Gender','Age','Mobile',
				 	'Email','Address','City','State','Note','Date');
				 foreach($patData as $i => $pat) {
						if($pat->device_type == "1"){
							$dType = "Android";
						}
	...
	public function runnersLeads(Request $request) {
		...
		$params = array();
			...
			if(!empty($request->input('created_by'))) {
				$query->whereHas('SalesTeam', function($q)  use ($created_by) {$q->Where('id', $created_by);});
			}
			if(!empty($search)) {
				$query->where(DB::raw('concat(name," ",IFNULL(mobile_no,""))'), 'like', '%'.$search.'%');
			}
