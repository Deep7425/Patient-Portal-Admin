    ...
    public function addBlog(Request $request){
		if($request->isMethod('post')) {
		 ...
		 // if($request->hasFile('video')) {
					...
					// $fullName = str_replace(" ","",$video->getClientOriginalName());
					// $onlyName = explode('.',$fullName);
					// if(is_array($onlyName)){
						// $videofileName = $onlyName[0].time().".".$onlyName[1];
					// }
					// else{
						// $videofileName = $onlyName.time();
					// }
					// $request->file('video')->move(public_path("/newsFeedFiles/videos"), $videofileName);
		 // }
             // $slug = preg_replace('/[^A-Za-z0-9-]+/', '-', trim(strtolower($data['title'])));
             // $slugs = NewsFeeds::select('slug')->where('slug', 'like', '%'.$slug.'%')->get();
             ...
             //  {
             //   $count = 0;
             //   while( in_array( ($slug . '-' . ++$count ), $slug_array) );
             //   $slug = $slug . '-' . $count;
             //  }
			NewsFeeds::create([
	...
	public function updateBlog(Request $request){
        if($request->isMethod('post')) {
		   ...
		   if ($request->hasFile('image')) {
			 ...
			 // if ($request->hasFile('video')) {
				 // $videofileName = public_path().'/newsFeedFiles/videos/'.$data['old_video'];
