	...
	public function getSubscriptionPlans(Request $request) {
		...
		if($validator->fails()){
			...
			if($user_array['lng'] == "hi") {
				$plans = Plans::Where(["delete_status"=>'1','status'=>1])->whereIn("type",array(3,4))->orderBy('price', 'ASC')->get();
	...
	public function checkMySubscription(Request $request)  {
	 ...
	 public function checkRefCode(Request $request)  {
		...
		if($validator->fails()){
			...
			$checkCode = User::select('id')->where(['mobile_no'=> $user_array['ref_code'],"parent_id"=>0])->where('id', '!=', $user_array['user_id'])->first();
			if(!empty($checkCode) && checkUserSubcription($checkCode->id)) {
				$success = true;
				if($user_array['lng'] == "hi") {
	  ...
	  public function downloadSubscriptionReceipt(Request $request){
			...
			if($validator->fails()){
				...
				$pdf_url = 	url("/")."/public/pdfviewforSubscription.pdf";
