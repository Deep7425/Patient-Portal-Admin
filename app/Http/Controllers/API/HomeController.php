...
class HomeController extends APIBaseController {
	...
	public function searchDoctorsByFilters(Request $request) {
		...
		dd($data);
  		$user_array=array();
		$user_array['sort_by_consult_fee'] = $data->get('sort_by_consult_fee');
		$user_array['consult_fee'] = $data->get('consult_fee');
		$user_array['consult_fee_max'] = $data->get('consult_fee_max');
		$user_array['speciality'] = $data->get('speciality');
		$user_array['group_id'] = $data->get('group_id');
		$user_array['search_key'] = $data->get('search_key');
		$user_array['city'] = $data->get('city');
		$user_array['locality'] = $data->get('locality');
		$user_array['search_type'] = $data->get('search_type');
		$user_array['gender'] = $data->get('gender');
		$user_array['rating'] = $data->get('rating');
		$user_array['conType'] = $data->get('conType');
		$user_array['by_home'] = $data->get('by_home');
		$doctors = [];
		$other_doctors = [];
		$suggested_data = [];
		$success = false;
	...
	public function getCountry(Request $request){
		$countryList =  Country::orderBy("id")->get();
		  ...
		  if($request->isMethod('post')) {
			...
			if($validator->fails()) {
				...
				if($user_array['old_number'] != $user_array['mobile_no']) {
					$no_exists = User::where(['mobile_no'=>trim($user_array['mobile_no'])])->count();
					if($no_exists > 0) {
				...
				if($user_array['old_number'] != $user_array['mobile_no']) {
					...
					if(!empty($user_array['mobile_no'])) {
					   ...
					   $this->sendSMS($user_array['mobile_no'],$message,'1707161587911566049');
					}
				...
				return $this->sendResponse($user, 'Profile update Successfully.',true);
			}
		}
	}
    	...
    	if($request->isMethod('post')) {
  		...
  		if($validator->fails()){
  			return $this->sendError('Validation Error.', $validator->errors());
				...
				// $this->uploadUserFileByApi($image,$image_old);
				...
				$user->organization = array("id"=>($user->organization!=null)?$user->organization:"","title"=>getOrganizationIdByName(@$user->organization),"logo"=>$logo);
				return $this->sendResponse($user, 'Patient Updated Successfully.',true);
			}
		 }
	}
	...
	public function addAppointment(Request $request) {
		...
		if($validator->fails()) {
			...
			if($user_array['isFirstTeleAppointment'] == 1 || $user_array['isDirectAppt'] == 1) {
				...
				// $isSubsAppt_free = 0;
				// $dt = date('Y-m-d');
				// $is_subscribed_user = PlanPeriods::select('id')->whereDate('start_trail','<=', $dt)->whereDate('end_trail','>=', $dt)->where('user_id', $user_array['order_by'])->where('remaining_appointment', '!=', '0')->where('status', '1')->count();
				
				// if($is_subscribed_user > 0){
					// $isSubsAppt_free = 1;
				// }
				...
				/*else if($isSubsAppt_free = 1 && $user_array['isFirstTeleAppointment'] == 1 && $user_array['isDirectAppt'] == 1 && $appointment > 0) {
...
public function checkFirstDirectTeleAppointment(Request $request) {
	...
	public function getDocDetailById(Request $request) {
		...
		if(!empty($user_array['doc_id'])) {
			$value = Doctors::Where(['delete_status'=>1])->where(["id"=>$user_array['doc_id']])->first();
		}
	...
	public function checkVersionOfApp(Request $request) {
		$data = Input::json();
		$xml=simplexml_load_file("app-update.xml") or die("Error: Cannot create object");
	...
	public function sendVideoNotification(Request $request) {
		$data = Input::json();
		$fcmToken = $data->get("fcm_token");	
	    if($fcmToken) {
			$api_key = "AAAAQ41aW-Y:APA91bH1ibH5FkF9nCE0MIYhUSPAORCjsGuUHT-zpJrRUbbnstuEtwepPtiKu4iF98W-1XX4vpDd6LAYczmqCkUSwYbdjcn8bHshHSAc4FjeJpUXFwYPpE2RQ0VBf7PifOr7e2KcCju3";
			// $notificationKey = "AAAAQ41aW-Y:APA91bH1ibH5FkF9nCE0MIYhUSPAORCjsGuUHT-zpJrRUbbnstuEtwepPtiKu4iF98W-1XX4vpDd6LAYczmqCkUSwYbdjcn8bHshHSAc4FjeJpUXFwYPpE2RQ0VBf7PifOr7e2KcCju3";
          ...
          $result = curl_exec($ch);
          curl_close( $ch );
		  return $this->sendResponse(strip_tags($result),'success',true);	
        }
    }
