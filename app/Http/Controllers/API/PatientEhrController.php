...
class PatientEhrController extends APIBaseController {
		...
		if($validator->fails()){
			...
			if(!empty($user_array['pid'])) {
				...
				// if(count($users)){
					...
					// }
				// }
				$user = Appointments::with(['user.doctorInfo','practiceDetails','AppointmentOrder','NotifyUserSms'])->whereIn('pID',$p_ids)->where('delete_status',1)->where('doc_id','!=',2219)->orderBy('start','desc')->paginate(12);
			}
			if(count($user) > 0) {
				foreach($user as $val) {
	...
	public function writeClinicNoteFile($patient_number,$prescription) {
		// $docPath = public_path().'/PatientDocuments/'.$patient_number.'/misc/';
		// if(!is_dir($docPath)){
	...
	public function uploadDocument(Request $request) {
		if($request->isMethod('post')) {
			...
			if($validator->fails()){
				...
				if($request->hasFile('document')) {
					...
					if($file_size_byte >= 1048576*15){
						return $this->sendError('File Size '.number_format($file_size_byte / 1048576, 15) . ' MB is exceed,(Max : 15 Mb Allowed)');
					}
					...
					$path_url = getPath("uploads/PatientDocuments/".$data['patient_number']);
					$file_arr = [];
					...
					// if(is_dir($path."/".$data['appointment_id'])) {
						...
						foreach($filesArr as $file_nm) {
							if(!empty($file_nm) && $file_nm != "htmltopdfview.pdf"){
								...
								$file_arr[] = array("document" => $path_url."/appointments/".$data['appointment_id']."/".$file_nm,'doc_name'=>$file_nm,"file_ext" => $file_extension);
							}
						}
					// }
		...
		public function getUserDocument(Request $request){
			if($request->isMethod('post')) {
				...
				if ($validator->fails()){
					...
					$path_url = getPath('uploads/PatientDocuments/'.$user_array['patient_number']);
					$file_arr = [];
					$filesArr = [];
					$S3AllFiles = Storage::disk('s3')->files($path);
					if(count($S3AllFiles)>0){
					...
					// if (is_dir($path."/".$user_array['appointment_id'])) { 
						...
						foreach($filesArr as $file_nm) {
							if(!empty($file_nm) && $file_nm != "htmltopdfview.pdf"){
								...
								$cnt = $file_ext_count - 1;
								$file_extension = $file_ext[$cnt];
								$file_arr[] = array("document" => $path_url."/appointments/".$user_array['appointment_id']."/".$file_nm,'doc_name'=>$file_nm,"file_ext" => strtolower($file_extension),"src"=>$path_url."/appointments/".$user_array['appointment_id']."/".$file_nm);
							}
						}
					// }
		...
		public function downloadReceipt(Request $request){
			...
			if($validator->fails()){
				...
				$pdf_url = 	url("/")."/public/pdfviewforAppointment.pdf";
				return $this->sendResponse($pdf_url,'',true);
			}
		}
				...
				// if(!empty($path."/".$user_array['appointment_id'])) {
					...
					foreach($filesArr as $file_nm) {
						if(!empty($file_nm) && $file_nm != "htmltopdfview.pdf"){
							...
							$cnt = $file_ext_count - 1;
							$file_extension = $file_ext[$cnt];
							$file_arr[] = array("document" => $path_url."/appointments/".$user_array['appointment_id']."/".$file_nm,'doc_name'=>$file_nm,"file_ext" => $file_extension,"src"=>$path_url."/appointments/".$user_array['appointment_id']."/".$file_nm);
						}
					}
				// }
		...
		public function latestappointmentfeedback(Request $request){
			...
			if($validator->fails()){
				...
				if(!empty($user_array['mobile_no'])){
					...
					$appointment = Appointments::with(['patient','AppointmentOrder','user.doctorInfo.docSpeciality','practiceDetails'])->whereIn('pId',$pIds)->where(["delete_status"=>1,"appointment_confirmation"=>1])->where('doc_id','!=',2219)
					...
					$appointment = Appointments::with(['patient','AppointmentOrder','user.doctorInfo.docSpeciality','practiceDetails'])->where(['pId'=>$user_array['pId'],"delete_status"=>1,"appointment_confirmation"=>1])->where('doc_id','!=',2219)
				...
				if($user_array['isFirstTeleAppointment'] == 1) {
					 if($user_array['lng'] == "hi") {
						return $this->sendResponse('','नि: शुल्क नियुक्ति के लिए कूपन कोड लागू नहीं है',false);
						 ...
						 return $this->sendResponse('','Coupon Code not applicable for free appointment',false);
					 }
				}
