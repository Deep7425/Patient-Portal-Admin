...
class LabController extends APIBaseController {
	...
	public function getthyrocareData(Request $request) {
		...
		public function getThyrocarePackageGroup(Request $request) {
			...
			if(count($groups) > 0) {
				...
				$profile_product = ThyrocareLab::where(['type'=>"PROFILE"])->get();
	...
	public function createLaborderAddresses(Request $request) {
			 ...
			 if($request->isMethod('post')) {
				...
				if($validator->fails()) {
					return $this->sendResponse('','Coupon Code Is Required',false);
					 ...
					 $success = false;
					 $dt = date('Y-m-d');
					 $coupon_data =  Coupons::where('coupon_code',$user_array['coupon_code'])->where(['type'=>1])->first();
		...
		public function getMyLabOrders(Request $request) {
			  ...
			  if($validator->fails()){
                return $this->sendError($validator->errors());
				  ...
				  $labs = LabOrders::with(["LabOrderTxn","Coupons","PlanPeriods.UserSubscribedPlans"])->where(["user_id"=>$user_array['user_id']])->orderBy("id","DESC")->where(["delete_status"=>"1","status"=>1])->whereNotNull("order_status")->get();
				    $success = false;
				    if(count($labs)>0) {
					   foreach($labs as $lab) {
						   $orderId = $lab->orderId;
					   ...
					   if($lab->type == 4) {
						   ...
						   $meta_data['items'] = [['lab_company'=>LabCompany::where('id',4)->where('status','1')->first()]];
		...
		public function getLabCartData(Request $request) {
			  ...
			  if($validator->fails()){
                return $this->sendError($validator->errors());
				  ...
				  $labCart = getLabCart($user_array['user_id']);
				  $success = false;
				  if(count($labCart)>0) {
					  $success = true;
				  }
		...
		public function addLabCartData(Request $request) {
				...
				$user_array=array();
				$user_array['user_id'] = $data->get('user_id');
				$user_array['products'] = $data->get('products');
					...
					foreach($user_array['products'] as $product) {
						...
						if($lab_exists == 0) {
							$LabCart = LabCart::create([
								'user_id' => $user_array['user_id'],
								'lab_id' => isset($product['lab_id'])?$product['lab_id']:null,
								'type' => $product['compid'],
		...
		public function deleteLabCartData(Request $request) {
				...
				$user_array=array();
				$user_array['user_id'] = $data->get('user_id');
				$user_array['product_name'] = $data->get('product_name');
				$user_array['product_code'] = $data->get('product_code');
				$user_array['lab_cart_type'] = $data->get('lab_cart_type');
		...
		public function createCustomLabOrder(Request $request) {
			if($request->isMethod('post')) {
				...
				$user_array['report_code'] = $data->get('ReportCode');
				$user_array['Gender'] = $data->get('Gender');
				$user_array['age'] = $data->get('age');
				$user_array['cmpId'] = $data->get('cmpId');
				$user_array['availWalletAmt'] = $data->get('availWalletAmt');
				// $user_array['Margin'] = (String) $data->get('Margin');
				// $user_array['Passon'] = $data->get('Passon');
				// $user_array['Remarks'] = $data->get('Remarks');
	...
	public function getDefLabByName(Request $request) {
		...
		if($validator->fails()){
			...
			$labs = DefaultLabs::with('LabCollection')->where('delete_status', '=', '1')->where(DB::raw('concat(default_labs.title," ",IFNULL(default_labs.short_name,""))') , 'like', '%'.$user_array['search'].'%')
			->get();
			$arr = [];
			if(count($labs)>0){
				foreach($labs as $raw){
	...
	public function getLabDataByComapny($comapny,$ids){
		if($comapny == 2) {
		 ...
		 $query = LabCollection::with("DefaultLabs")->where(['company_id'=>$comapny,'delete_status'=> '1']);
	...
	public function getLabPackage(Request $request) {
		...
		if($validator->fails()){
			...
			if($user_array['comp_id'] == "2") {
				...
				// $product = ThyrocareLab::where(['type'=>"OFFER"])->get()->toArray();
				$rate = array();
				foreach($product as $key => $row) {
					$rate[$key] = $row->rate->offerRate;
				}
				...
				$product = LabPackage::where('company_id',$user_array['comp_id'])->where(['status'=>1,'delete_status'=>1])->orderBy('id','desc')->get();
				if($product->count()>0) {
