...
<input class="gLabCmptp" type="hidden" value='{{ Session::get("lab_company_type") }}'/>
...
<div class="navbar-header">
  ...
  <a class="navbar-brand" href="{{route('index')}}"><img width="144" height="60" src="{{ URL::asset('img/logo.webp') }}" alt="short-logo" /></a>
</div>
...
@if(Auth::user())
	...
	<li class="dropdown sub-nev-tool">
	  <a class="dropdown-toggle" role="button" data-toggle="dropdown" href="javascript:void(0);"><img class="top-user-img" @if($image_url != null) src="{{$image_url}}" @else src="{{ URL::asset('img/user-icon.png') }}" @endif/><span class="user-nametop">@if(!empty($user->first_name)) {{@$user->first_name}} {{@$user->last_name}} @else {{@$user->mobile_no}} @endif</span><span class="caret"></span>
	  </a>
	...
	<ul class="cart-top">
    	<li class="medicine main-nav-desktop cart-wrapper hideforPaytmCart"  data-toggle="tooltip" title="@if(!empty($LabCart)) Cart @else Cart is Empty! @endif">
			<a href="{{ route('LabCart') }}"><img src="{{ URL::asset('img/cart-new.png') }}" alt="cart-icon" /><div class="nav-section"><p> <span class="cartTotal" id="cartTotal">@if(!empty($LabCart)) {{count($LabCart)}} @else 0 @endif</span> </p></div></a>
					...
					<div id="miniCartList">
						@if(!empty($LabCart))
							...
							@if(isset($package['DefaultLabs']))
								...
								<div class="list" title="{{$package['DefaultLabs']['title']}}" lab_type="1" lab_company_type="1" data-name="{{$package['DefaultLabs']['title']}}"><img width="15" height="15" src="{{asset('img/OurStore-icon.png')}}" alt="store-icon"/><h5> <a href="{{route('LabDetails', ['id'=> base64_encode($package['DefaultLabs']['title']), 'type' => base64_encode($type)])}}">{{$package['DefaultLabs']['title']}}</a></h5> <span><strong>1 x ₹ @if(isset($package['offer_rate'])) {{@$package['offer_rate']}} @else {{@$package['cost']}} @endif </strong></span>
							...
							@else
								<div class="list" title="{{$package['name']}}" lab_type="0" lab_company_type="0" data-name="{{$package['name']}}"><img width="15" height="15" src="{{asset('img/OurStore-icon.png')}}" alt="store-icon" /><h5> <a href="{{route('LabDetails', ['id'=> base64_encode($package['name']), 'type' => base64_encode($package['type'])])}}">{{$package['name']}}</a></h5> <span><strong>1 x ₹ @if($package['rate']['offerRate'] == "null") {{$package['rate']['b2C']}} @else {{$package['rate']['offerRate']}} @endif </strong></span>
  ...
  <ul class="nav navbar-nav navbar-right">
    ...
    <li class="lab-test main-nav-desktop"><a href="{{route('LabDashboard')}}" ><div class="icon-top"><img width="35" height="35" src="{{ URL::asset('img/lab-icon-top.png') }}" alt="lab-icon" /></div><div class="nav-section"><h3>Lab Test</h3><p>Book Health Packages</p></div></a>
  	...
  	<li class="active apoointment main-nav-desktop" style=" width: 190px;">
		<a  class="dd" href="{{route('covidGuide')}}"><span class="text" style="display:none;">COVID</span><div class="icon-top"><img src="{{ URL::asset('img/covid-icon.png') }}" width="46" height="43" alt="covid-icon" /></div><div class="nav-section"><h3>Covid Guide</h3><p>Information Center</p></div></a>
	</li>
	<li class="active apoointment main-nav-desktop lab-test">
        ...
        <div class="icon-top"><img width="35" height="35"  src="{{ URL::asset('img/appointment-ico-top.webp') }}" alt="appointment-icon" /></div><div class="nav-section"><h3>Find Doctors</h3><p>Book Appointment</p></div></a>
	</li>
...
<div class="navbaar-bottom  mobile-search">
	@if($controller == "LabController")
		...
		<button type="button" class="btn btn-default searchLabModalDoctor" data-toggle="modal" ><i class="" aria-hidden="true"><img width="20" src="{{ URL::asset('img/doctor-mail.png') }}" alt="doctor-mail" /></i>Search Lab Tests</button>
	@else
	...
	<button type="button" class="btn btn-default searchDoctorModalDoctor" data-toggle="modal" ><i class="" aria-hidden="true"><img width="20" height="19" src="{{ URL::asset('img/doctor-mail.png') }}" alt="doctor-mail" /></i>Search Doctors/Specialties</button>
	@endif
</div>
...
<div class="collapse main-nav-mobile-lab container-fluid">
	<div class="container">
	    ...
	    <li class="active apoointment main-nav-desktop lab-test">
			<a search_type="1" data_id="0" info_type="wallet" class="dd wallet walletbutton" @if($controller == "LabController") href="{{route('index')}}" @else href="javascript:void(0);" @endif ><span class="text" style="display:none;">User</span><div class="icon-top"><img width="35" height="35" src="img/appointment-ico-top.webp" alt="appointment-icon" /></div><div class="nav-section"><h3>Wallet</h3><p>@if(@$user->userDetails->wallet_amount) {{@$user->userDetails->wallet_amount}} @else 0.00 @endif </p></div></a>
		</li>
		...
		<div class="navbaar-bottom-section">
			<div class="navbaar-bottom-box2">
				<div class="navbaar-bottom-box">
					<input type="search" class="labSearching" placeholder="Search Lab CBC Thyroid Full Body checkup..." value="{{ Session::get('search_from_lab') }}" name="lab_search" autocomplete="off" />
...
<div class="modal fade" id="searchLabModalDoctor" role="dialog" data-backdrop="static" data-keyboard="false">
	<div class="container">
		...
		<div class="navbaar-bottom-box2">
			<div class="navbaar-bottom-box">
				<input type="search" class="labSearching" placeholder="Search Lab CBC Thyroid Full Body checkup..." value="{{ Session::get('search_from_search_bar') }}" name="lab_search" autocomplete="off" />
				<button type="button" class="btn btn-default search_close_lab" style="display:none;"><i class="fa fa-times" aria-hidden="true"></i></button>
				<div class="dd-wrapper labSearchByInput" style="display:none;"></div>
			</div>
		</div>
