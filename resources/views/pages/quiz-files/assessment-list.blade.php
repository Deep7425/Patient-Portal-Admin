            ...
            <div class="layout-container">
                ...
                <div class="layout-content">
                    ...
                    <div class="container-fluid flex-grow-1 container-p-y">
                        <h4 class="font-weight-bold py-3 mb-0">Health Assessment List</h4>
                        <!--<div class="text-muted small mt-0 mb-4 d-block breadcrumb">
                         ...
                         <!-- <div id="bootstrap-table-toolbar">
                                    ...
                                    <div class=" dd-wrapper">
                                    	<div id="bootstrap-table-toolbar">
                                          <form id="chnagePagination" action="{{route('assessmentList',$orgData->slug)}}">
										  @csrf
                            ...
                            <form id="counceling" action="{{url('counciling')}}">
                            <div class="talbeBlock">
                            <table class="table table-hover" style="width: 1037.17px;"><thead><tr><th>SR No </th><th><input class="custom_css" onclick="CheckAll(this)" type="checkbox" name="quiz_id[]" ></th><th>Name</th><th>Gender</th><th>Mobile</th><th>Age</th><th>Negative Effect</th><th>Detachment</th><th>Antgonism</th><th>Disinhibition</th><th>Psuchoticism</th><th>Total Score</th><th>Status</th></tr>
                            <?php $i=1; ?>
                            @foreach($enquirys as $enquirysres)

                           <tr>
                            <td>{{$i}}  <label></td><td><input class="custom_css"  type="checkbox" name="quiz_id[]" value="{{$enquirysres->id}}"> </label></td><td><a href="{{url("/")}}/report-card/{{base64_encode($enquirysres->id)}}/{{$orgData->slug}}">{{$enquirysres['name']}}</a></td><td>{{$enquirysres['gender']}}</td><td>{{$enquirysres['mobile']}}</td><td>{{$enquirysres['age']}}</td><td>{{$enquirysres['negative_effect']}}</td><td>{{$enquirysres['detechment']}}</td><td>{{$enquirysres['antagonism']}}</td><td>{{$enquirysres['disinhibition']}}</td><td>{{$enquirysres['psychoticism']}}</td>
                            <td>{{$enquirysres['finalTotalScore']}}</td>
                            <td>@if($enquirysres['status']==1) Normal @endif
                                @if($enquirysres['status']==2) Mild @endif
                                
                                @if($enquirysres['status']==3) Most Vulnerable @endif
                            </td>
