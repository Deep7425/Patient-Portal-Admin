...
<div class="modal fade myModal-enquiry" id="walletmodel" role="dialog" data-backdrop="static" data-keyboard="false">
  
  <div class="modal-dialog">
  <div class="modal-content">
    <div class="modal-header">
      <div id="success-alert" style="display:none"><p class="alert alert-success">Wallet Applied successfully</p></div>
      <h4 class="modal-title"><a href="#"><img width="180" height="75" src="{{ URL::asset('img/logo.png') }}" /></a></h4>
      <h2>You can share also</h2>
    </div>
    <div class="modal-body">
      ...
      {{-- <div class="items-section" id="name">
        <label>Wallet Code:<i class="required_star">*</i></label>
        <input type="text" id="wallet_code"  onclick="copyToclipboard()" class="form-control wallet_code"  value="@if(@$user->userDetails->referral_code) {{@$user->userDetails->referral_code}} @else 0.00 @endif"  name="wallet_code"/>
		<input type="hidden" class="form-control"  name="wallet_amount"/>
  ...
  width: 50%;
  padding: 10px;">
 <p><a href="https://web.whatsapp.com/send?text=www.google.com" target="_blank">Share to your fiend via Whatsapp</a></p>
	    <p style="display:none; color:green" id="toCoppied">Wallet Code coppied</p>
    <input type="text" id="text-1" value="@if(@$user->userDetails->referral_code){{@$user->userDetails->referral_code}}@else 0.00 @endif" />
      <button onclick="copy('text-1')">Copy Wallet Code</button>
    </div>
    </div>
  </div>
  {{-- <div class="modal-footer">
    <button name ="submit" type="submit"  class="btn btn-default submitBtn">SUBMIT</button>
    <button name ="clear" type="button" class="btn btn-default closewalletPopup" data-dismiss="modal">NO THANKS</button>
  </div> --}}
</div>
</div>
</div>
...
jQuery(document).ready(function(){
	 jQuery("#enquiryFromSubmit").validate({
        ...
        if (newMobile.charAt(0) == '0') {
            $('.error').html('Mobile number can\'t start from zero');
			alert("Zero is not allowed at first digit of mobile no");
			return false;
       }
			jQuery.ajax({
			type: "POST",
