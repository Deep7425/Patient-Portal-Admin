		...
		<div class="modal-content ">
			...
			<div class="modal-body">
				<div class="panel panel-bd lobidrag">
					...
					<div class="panel-body">
						{!! Form::open(array('id' => 'sendUserBulkSms','name'=>'sendUserBulkSms','enctype' => 'multipart/form-data')) !!}
						<input name="ids" value="" id="ids" type="hidden"/>
						<input name="nType" value="1" id="nType" type="hidden"/>
						<input name="Allids" value="{{json_encode($AllPatientsIds)}}" id="Allids" type="hidden"/>
							...
							<div class="col-sm-4">
						<div class="form-group CampaignType">
							<label>Campaign Type</label>
							<p class="Notification1234">
						...
						<div class="form-group notificationDiv " style="display:none;">
							<label>Application Page</label>
							<!-- <select name="notification_route" class="form-control notificationItem">
							<option value="">Select</option>
							@forelse(getAppPages() as $raw)
								<option value="{{$raw->slug}}">{{$raw->name}}</option>
							@empty

								<option value="" class = "">Select</option>
							@endforelse
							<option value="{{$raw->slug}}">type here</option>
							</select> -->
							<input type="text" disabled class="form-control" name="notification_route" id="urlInput" style="display: none;" placeholder="Enter URL"/>	
							<select name="notification_route" class="form-control notificationItem" id="notificationRouteSelect">
							<option value="">Select</option>
							@forelse(getAppPages() as $raw)
								<option value="{{$raw->slug}}">{{$raw->name}}</option>
							@empty
								<option value="">Select</option>
							@endforelse
							<option value="0">For URL</option>
							</select>
							<span class="help-block"></span>
						</div>
						</div>
						<div class="col-sm-4">
						<div class="form-group doctorSectionDiv doctorSectionDiv1234" style="display:none;">
							<label>Doctors List</label>
							<select class="form-control selectDocNotify" name="user_id">
								<option value="" selected>Select Doctor</option>
								@if(count($practices) > 0)
									@foreach($practices as $pra)
										<option value="{{$pra->id}}">{{@$pra->first_name}} {{@$pra->last_name}}</option>
									@endforeach
								@endif
							</select>
						...
						<div class="col-sm-4">
						<div class="form-group planSectionDiv doctorSectionDiv1234" style="display:none;">
							<label>Plan List</label>
							<select class="form-control selectPlanNotify" name="plan_id">
							...
							<span class="help-block"><label for="planSelectMultiple" generated="true" class="error" style="display:none;"></label></span>
						</div>
						</div>
						<div class="col-sm-4">
						<div class="form-group labSectionDiv doctorSectionDiv1234" style="display:none;">
							<label>Lab List</label>
							<select class="form-control selectLabNotify" name="lab_id">
								<option value="">Select Lab</option>
								@if(count($labpackages) > 0)
									@foreach($labpackages as $labpackage)
										<option value="{{$labpackage->id}}">{{@$labpackage->title}}</option>
									@endforeach
								@endif
							</select>
							<span class="help-block"><label for="labSelectMultiple" generated="true" class="error" style="display:none;"></label></span>
						</div>
						</div>
						<div class="col-sm-4">
						<div class="form-group symptomSectionDiv doctorSectionDiv1234" style="display:none;">
							<label>Symptom List</label>
							<select class="form-control selectSymptomNotify" name="symptom_id">
								<option value="">Select Symptom</option>
								@if(count($symptomslist) > 0)
									@foreach($symptomslist as $symptom)
										<option value="{{$symptom->id}}">{{@$symptom->symptom}}</option>
									@endforeach
								@endif
							</select>
							<span class="help-block"><label for="symptomSelectMultiple" generated="true" class="error" style="display:none;"></label></span>
						</div>
						</div>
						...
						<div class="form-group notificationDiv">
							...
							<input name="notification_icon" class="form-control" type="file"/>
							<span class="help-block"></span>
						</div>
						<div class="form-group notificationDiv" style="display:none;">
		...
		<div class="modal-body">
			<div class="panel panel-bd lobidrag">
				<div class="panel-body SwitchAppointment">
					...
					<div class="form-group">
						<label>Doctors</label>
						<?php $specialist_doctor_ids = getSetting("specialist_doctor_user_ids");?>
						<select class="form-control chooseDoc" name="doc_id">
							...
							@if(count($practices) > 0)
								@foreach($practices as $pra)
									...
									@if(in_array($pra->user_id,$specialist_doctor_ids))(PLAN)@endif</option>
									@endif
								@endforeach
							@endif
						</select>
...
$(document.body).on('change', '.userTypeFilter', function(){
 ...
 jQuery(document).on("click", ".smsType", function () {
	if($(this).val() == '2'  && $(this).prop("checked") == true){
	  ...
	  $(".doctorSectionDiv").hide();
	  $(".planSectionDiv").hide();
	  $(".labSectionDiv").hide();
	  $(".symptomSectionDiv").hide();
	  $('.notificationItem').find('option').attr('selected',false);
	  $('.selectDocNotify').val('');
	  $('.selectDocNotify').trigger('change');
	  $('.selectPlanNotify').val('');
	  $('.selectPlanNotify').trigger('change');
	  $('.selectLabNotify').val('');
	  $('.selectLabNotify').trigger('change');
	  $('.selectSymptomNotify').val('');
	  $('.selectSymptomNotify').trigger('change');
    }
});
jQuery(document).on("change", ".notificationItem", function () {
	console.log($(this).val());
	if($(this).val() == 'doctor-detail' || $(this).val() == 'feedback'){
		$(".doctorSectionDiv").show();
		$(".planSectionDiv").hide();
		$(".labSectionDiv").hide();
		$(".symptomSectionDiv").hide();
    }
	else if($(this).val() == 'buy-plan'){
		$(".planSectionDiv").show();
		$(".doctorSectionDiv").hide();
		$(".labSectionDiv").hide();
		$(".symptomSectionDiv").hide();
    }
	else if($(this).val() == 'offer-desc'){
		$(".labSectionDiv").show();
		$(".doctorSectionDiv").hide();
		$(".planSectionDiv").hide();
		$(".symptomSectionDiv").hide();
    }
	else if($(this).val() == 'symptom-detail'){
		$(".symptomSectionDiv").show();
		$(".doctorSectionDiv").hide();
		$(".planSectionDiv").hide();
		$(".labSectionDiv").hide();
    }
...
$(document).ready(function() {
	...
	$(".chooseDoc").select2();
	$(".selectDocNotify").select2();
	$(".selectPlanNotify").select2();
	$(".selectLabNotify").select2();
	$(".selectSymptomNotify").select2();
});
...
$(document).ready(function() {
    $('#notificationRouteSelect').on('change', function() {
        var selectedValue = $(this).val();
        if (selectedValue === '0') {
            $('#urlInput').show();
			$('#urlInput').attr('disabled',false);
            // $(this).replaceWith($('#urlInput'));
			$('#notificationRouteSelect').attr('disabled',true);
			$('#notificationRouteSelect').hide();
            $("#nType").val(2);
        }
		else{
			$('#urlInput').hide();
			 $('#urlInput').attr('disabled',true);
			$('#notificationRouteSelect').attr('disabled',false);
			$('#notificationRouteSelect').show();
            // $(this).replaceWith($('#urlInput'));
            $("#nType").val(1);
		}
    });
